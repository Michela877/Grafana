...
Questa guida descrive i passaggi per configurare l’ambiente di monitoraggio `BlackBox - prometheus` su Kubernetes.

---

## 1️⃣ Creazione dell'ingress controller

utilizzare questo comando per installare il tutto 

```bash
kubectl apply -k "path assoluto in locale" -n monitoring
```

## 2️⃣ Creazione dei singoli con kustomize
una volta lanciato il deployment bisognera installare questa parte sulla configmap di prometheus alla fine in questo caso gia è esistente in questo repositroy l'unica cosa da aggiungere sono i siti da monitorare sotto la relabel_configs
...
## 3️⃣ configurazione dashboard Grafana Prometheus

QUI SOTTO DASHBOARD PER Blackbox PER I gli status dei siti da abbinare con email se un sito va in 404
questo e codice dashboard 7587 per sicurezza questo e il codice yaml 
